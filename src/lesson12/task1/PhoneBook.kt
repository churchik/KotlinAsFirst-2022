...
class PhoneBook {
    ...
    fun addHuman(name: String): Boolean {
        if (book.keys.contains(name)) return false
        else {
    ...
    fun addPhone(name: String, phone: String): Boolean {
        for ((key, value) in book) if (value.contains(phone)) return false
        if (name !in book.keys) return false
        else book[name]?.add(phone)
        return true
    }
    ...
    fun removePhone(name: String, phone: String): Boolean {
        for ((key, value) in book) if (key == name && !value.contains(phone)) return false
        if (name !in book.keys) return false
        else book[name]?.remove(phone)
        return true
    }
    ...
    fun phones(name: String): Set<String> {
        if (name !in book.keys || book[name]!!.isEmpty()) return setOf()
    ...
    fun humanByPhone(phone: String): String? {
        for ((key, value) in book) if (value.contains(phone)) return key
        return null
    }
    ...
    override fun equals(other: Any?): Boolean {
        val set1 = mutableSetOf<Pair<String, List<String>>>()
        val set2 = mutableSetOf<Pair<String, List<String>>>()
        if (other is PhoneBook) for ((key, value) in other.book) set2.add(Pair(key, value.sorted()))
        else return false
        for ((key, value) in book) set1.add(Pair(key, value.sorted()))
        return set1 == set2
    }
