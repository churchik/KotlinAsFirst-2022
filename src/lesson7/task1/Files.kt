...
fun deleteMarked(inputName: String, outputName: String) {
    File(outputName).bufferedWriter().use {
        for (str in File(inputName).readLines()) {
            if (str.isEmpty() || !str.startsWith('_')) it.write(str + "\n")
        }
    }
}
...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    val uniqueList = substrings.distinctBy { it }
    for (line in File(inputName).readLines()) {
        val low = line.lowercase()
        for (str in uniqueList) {
            if (!map.contains(str)) map[str] = 0
            for (step in line.indices) {
                if ((low.startsWith(str.lowercase(), step))) map[str] = map[str]!! + 1
            }
        }
    }
    return map
...
fun centerFile(inputName: String, outputName: String) {
    ...
    File(outputName).bufferedWriter().use {
        ...
        for (line in File(inputName).readLines()) {
            if (max != null) {
                it.write(" ".repeat((max - line.trim().length) / 2))
            }
            it.write(line.trim())
            it.newLine()
        }
...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    File(outputName).bufferedWriter().use { out ->
        if (input.isEmpty() || File(inputName).readText() == "") {
            out.write("")
            return
        }
        val max = input.maxOf { it.trim().length }
        for (line in input) {
            val splitLineToWords = line.trim().split("""\s+""".toRegex()).toMutableList()
            val trimLine = splitLineToWords.joinToString(" ")
            if (trimLine.isBlank()) {
                out.newLine()
                continue
            }
